...
export class ViewController {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('admin')
  @Render('pages/admin-sites')
  adminSites(@Request() req) {
    const user = this.getUserFromRequest(req);
    return {
      title: 'Site Management - ZBase',
      user,
      path: '/admin/sites'
    };
  }
  ...
  adminPosts(@Request() req) {
    const user = this.getUserFromRequest(req);
    return {
      title: 'Post Management - ZBase',
      user,
      path: '/admin/posts'
    };
  }
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('admin', 'editor')
  @Render('pages/admin-post-view')
  adminPostView(@Request() req, @Param('id') id: string) {
    const user = this.getUserFromRequest(req);
    return {
      title: 'View Post - ZBase',
      user,
      postId: id,
