...
Dự án ZBase có thể được chạy theo hai cách:

1. **Sử dụng Docker**: Phương pháp đề xuất, không cần cài đặt Node.js hay PostgreSQL trên máy host
2. **Cài đặt trực tiếp**: Phương pháp truyền thống, yêu cầu cài đặt Node.js và PostgreSQL
...
DB_HOST=127.0.0.1  # Sẽ tự động được ghi đè thành "postgres" khi chạy trong Docker
DB_PORT=5432
...
Chạy ứng dụng trong chế độ development (có hot-reload):
...
Để biết thêm chi tiết về cấu hình Docker, vui lòng xem [Hướng dẫn Docker](docker-readme.md).
...
### 8. Kiểm Tra Kết Nối Cơ Sở Dữ Liệu

Bạn có thể kiểm tra kết nối cơ sở dữ liệu bằng script:

```powershell
node test-connection.js
```

Script này sẽ hiển thị thông tin về kết nối và liệt kê các bảng trong cơ sở dữ liệu.
...
- **Trang Đăng Nhập**: http://localhost:3001/login
- **Trang Quản Trị**: http://localhost:3001/admin (yêu cầu đăng nhập với quyền admin)
