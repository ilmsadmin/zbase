...
export default function CreateWarrantyPage() {
  ...
  const [formData, setFormData] = useState<CreateWarrantyDto>({
    status: WarrantyStatus.PENDING,
    receivedDate: new Date(),
    charged: false,
    creatorId: 1 // This should come from authentication context
  });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setGeneratedCode(`WR${new Date().getTime().toString().substring(0, 10)}`);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (type === 'number') {
      ...
      setFormData({ ...formData, [name]: isNaN(parsedId) || parsedId === 0 ? undefined : parsedId });
    } else {
  ...
  const handleDateChange = (name: string, value: string) => {
    if (!value) {
      ...
      delete newFormData[name as keyof CreateWarrantyDto];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await warrantyService.createWarranty(formData);
      setSuccess(t("success.created"));
      
      // Redirect to the warranty detail page after successful creation
      setTimeout(() => {
        router.push(`/admin/warranties/${response.id}`);
      }, 1500);
    } catch (err) {
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <select
                    id="status"
                    name="status"
                    value={formData.status || WarrantyStatus.PENDING}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="customerId">
                    {t("form.customer")} <span className="text-red-500">*</span>
                  </label>
            ...
            {/* Diagnosis and Solution (Optional for new warranty) */}
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <div className="grid grid-cols-1 gap-6">
                <div>
                  ...
                  ></textarea>
                  <p className="mt-1 text-xs text-gray-500">{t("form.optionalForNewWarranty")}</p>
                </div>
